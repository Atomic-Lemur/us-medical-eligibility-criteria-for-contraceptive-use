...
async function writePDFAsync(base64: string) {
    await writeAsStringAsync(pdfPath, base64.replace('data:application/pdf;base64,', ''), {
...
const validate = ({
    ...
    if (!renderType || !source) {
        ...
        (!source.uri ||
            !(source.uri.startsWith('http') || source.uri.startsWith('file') || source.uri.startsWith('content')))
    ) {
        ...
        (!source.base64 || !source.base64.startsWith('data:application/pdf;base64,'))
    ) {
...
const PdfViewer = ({
    ...
    useEffect(() => {
        if (source.uri || source.base64) {
            setReady(false);
            setRenderType(getRenderType({ source, useGoogleDriveViewer, useGoogleReader }));
        }
